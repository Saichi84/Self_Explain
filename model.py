...
class ExplainableModel(nn.Module):
    def __init__(self, bert_dir):
        ...
        self.span_info_collect = SICModel(default_hidden_size)
        self.interpretation = InterpretationModel(default_hidden_size)
        self.output = nn.Linear(default_hidden_size, 2)

    def forward(self, input, start_indexs, end_indexs, span_masks):
        # # generate mask
        ...
        h_ij = self.span_info_collect(input, start_indexs, end_indexs)
...
class SICModel(nn.Module):
    ...
    def forward(self, hidden_states, start_indexs, end_indexs):
        ...
        W1_hi_ti_emb = torch.index_select(W1_h, 1, start_indexs)  # (bs, span_num, hidden_size)
        W1_hi_pj_emb = torch.index_select(W1_h, 1, end_indexs)
        W2_hj_ti_emb = torch.index_select(W2_h, 1, start_indexs)
        W2_hj_pj_emb = torch.index_select(W2_h, 1, end_indexs)
        ...
        span= (W1_hi_ti_emb+W1_hi_pj_emb)+(W2_hj_ti_emb-W2_hj_pj_emb)
